...
*/
void handle_shutdown(int sig, Shared* shared) {
    if (shared->listener_d)
...
*/
Shared* open_listener_socket(void) {
...
{
    int res = send(socket, s, strlen(s)-2, 0);
    if (res == -1)
...
int main(int argc, char *argv[])
{
    ...
    int port = 30000;
    Shared* shared = open_listener_socket();
    bind_to_port(shared->listener_d, port);

    if (listen(shared->listener_d, 10) == -1)
        error("Can't listen");
